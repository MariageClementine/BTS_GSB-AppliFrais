...
function ajouterLigneHF($idCnx, $unMois, $unIdVisiteur, $uneDateHF, $unLibelleHF, $unMontantHF) {
    ...
    $unMois = filtrerChainePourBD($unMois);
    $requete = "insert into LigneFraisHorsForfait(idVisiteur, mois, date, libelle, montant, etat) 
                values ('" . $unIdVisiteur . "','" . $unMois . "','" . $uneDateHF . "','" . $unLibelleHF . "'," . $unMontantHF .",0)";
    mysql_query($requete, $idCnx);
}

/**
 * Refuse une ligne hors forfait.
 * met a jour dans la BD le champ etat a 1 pour la ligne choisie 
 * $id correspondant à l'id de cette ligne
 * @param string $id identifiant de la ligne a refuser
 * @return void
 */
function refusLigneHF($id)
{
    $requete = "update LigneFraisHorsForfait set etat=1 where id=".$id;
    mysql_query($requete);
}
...
function verifierInfosConnexion($idCnx, $unLogin, $unMdp) {
    ...
    $req = "select id, nom, prenom, login, mdp,comptable from Visiteur where login='".$unLogin."' and mdp='" . $unMdp . "'";
 ...
 * @return void 
 */
function modifierEtatFicheFrais($unMois, $unIdVisiteur, $unEtat,$unNbJustificatifs) {
    if($unEtat != null)
    {
    ...
    if($unNbJustificatifs != null)
    {
       $requete = "update FicheFrais set nbJustificatifs = " . $unNbJustificatifs . 
               ", dateModif = now() where idVisiteur ='" .
               $unIdVisiteur . "' and mois = '". $unMois . "'";
    mysql_query($requete); 
    }
